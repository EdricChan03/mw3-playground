...
import {argbFromHex, DynamicColor, Hct, hexFromArgb, MaterialDynamicColors, rgbaFromArgb, SchemeContent} from '@material/material-color-utilities';
...
export const hexFromDynamicColor = (
  sourceColor: string, dynamicColor: DynamicColor, isDark: boolean
) => {
  const scheme = new SchemeContent(Hct.fromInt(argbFromHex(sourceColor)), isDark, 0);
...
export function themeFromSourceColor(color: string, isDark: boolean): Theme {
  ...
  for (const [key, value] of Object.entries(materialColors)) {
    ...
    // Generate additional raw components for TailwindCSS support
    // (see https://tailwindcss.com/docs/customizing-colors#using-css-variables)
    const { r, g, b } = rgbaFromArgb(value.getArgb(scheme));
    theme[`${key}--raw`] = `${r} ${g} ${b}`;
  }
  return theme;
}
